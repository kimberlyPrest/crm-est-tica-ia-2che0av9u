...
export default function FilesPage() {
    ...
    const handleDelete = async (id: string, fileName: string) => {
        const confirm = window.confirm(`Deseja deletar permanentemente o arquivo ${fileName}?`)
        if (!confirm) return
    ...
    return (
        <div className="space-y-6 animate-fade-in relative">
            ...
            <div className="flex justify-end mb-4">
                <AppButton onClick={() => toast.info('Funcionalidade de upload em breve')}>
                    ...
                    <GlassCard key={file.id} className="flex flex-col justify-between">
                        <div>
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <h3 className="font-bold text-gray-800 line-clamp-2" title={file.name}>{file.name}</h3>
                            </div>
                            <div className="space-y-2 mt-4">
                                <p className="text-xs text-gray-500">
                                    <span className="font-semibold">Status:</span> {file.is_active ? '✅ Ativo' : '❌ Inativo'}
                                </p>
                        ...
                        <div className="mt-4 flex gap-2 justify-end border-t border-gray-100 pt-4">
                            <AppButton variant="ghost" className="text-red-500 hover:text-red-600 hover:bg-red-50" onClick={() => handleDelete(file.id, file.name)}>
