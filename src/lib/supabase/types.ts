...
//   whatsapp_instance_id: uuid (nullable)
//   organization_id: uuid (not null, default: get_auth_org_id())
// Table: agent_config
//   id: uuid (not null, default: gen_random_uuid())
//   is_enabled: boolean (not null, default: true)
//   auto_schedule_enabled: boolean (not null, default: false)
//   auto_schedule_start_time: time without time zone (nullable)
//   auto_schedule_end_time: time without time zone (nullable)
//   agent_name: text (not null, default: 'Assistente'::text)
//   role_definition: text (nullable)
//   company_info: text (nullable)
//   tone: text (nullable)
//   knowledge_instructions: text (nullable)
//   guardrails: text (nullable)
//   human_handover_rules: text (nullable)
//   few_shot_examples: jsonb (nullable, default: '[]'::jsonb)
//   created_at: timestamp with time zone (nullable, default: now())
//   updated_at: timestamp with time zone (nullable, default: now())
//   gemini_api_key: text (nullable)
//   active_whatsapp_instance_id: uuid (nullable)
//   organization_id: uuid (not null, default: get_auth_org_id())
// Table: appointments
//   id: uuid (not null, default: gen_random_uuid())
...
//   type: text (not null, default: 'evaluation'::text)
//   notes: text (nullable)
//   created_at: timestamp with time zone (nullable, default: now())
//   updated_at: timestamp with time zone (nullable, default: now())
//   organization_id: uuid (not null, default: get_auth_org_id())
...
//   leads_waiting_human: bigint (nullable)
//   confirmed_appointments: bigint (nullable)
//   unread_notifications: bigint (nullable)
// Table: deal_sessions
//   id: uuid (not null, default: gen_random_uuid())
...
//   discount_value: integer (nullable, default: 0)
//   payment_method: text (nullable)
//   purchase_date: date (not null, default: CURRENT_DATE)
//   total_sessions: integer (not null)
//   completed_sessions: integer (not null, default: 0)
//   status: text (not null, default: 'active'::text)
//   next_session_due: date (nullable)
//   created_at: timestamp with time zone (nullable, default: now())
//   updated_at: timestamp with time zone (nullable, default: now())
//   organization_id: uuid (not null, default: get_auth_org_id())
...
//   trigger_keywords: _text (nullable, default: '{}'::text[])
//   is_active: boolean (not null, default: true)
...
//   ai_agent_blocked: boolean (not null, default: false)
//   has_pending_message: boolean (not null, default: false)
//   last_interaction_at: timestamp with time zone (nullable, default: now())
//   notes: text (nullable)
...
//   return_interval_days: integer (not null, default: 30)
...
//   connection_error_message: text (nullable)
//   created_at: timestamp with time zone (nullable, default: now())
//   updated_at: timestamp with time zone (nullable, default: now())
...
//   UNIQUE cadence_templates_name_organization_id_key: UNIQUE (name, organization_id)
...
//   UNIQUE products_name_organization_id_key: UNIQUE (name, organization_id)
...
// Table: status
//   UNIQUE status_name_organization_id_key: UNIQUE (name, organization_id)
...
//   UNIQUE whatsapp_instances_organization_id_key: UNIQUE (organization_id)
...
//     WITH CHECK: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = activities.organization_id)))
//   Policy "Users can only see activities in their organization" (SELECT, PERMISSIVE) roles={public}
//     USING: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = activities.organization_id)))
// Table: agent_config
//   Policy "Tenant Isolation" (ALL, PERMISSIVE) roles={public}
//     USING: (organization_id = get_auth_org_id())
//     WITH CHECK: (organization_id = get_auth_org_id())
//   Policy "Users can delete agent_config in their organization" (DELETE, PERMISSIVE) roles={public}
//     USING: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = agent_config.organization_id)))
//   Policy "Users can insert agent_config in their organization" (INSERT, PERMISSIVE) roles={public}
//     WITH CHECK: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = agent_config.organization_id)))
...
//     USING: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = appointments.organization_id)))
//   Policy "Users can insert appointments in their organization" (INSERT, PERMISSIVE) roles={public}
//     WITH CHECK: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = appointments.organization_id)))
...
//     USING: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = cadence_templates.organization_id)))
//   Policy "Users can insert cadence templates in their organization" (INSERT, PERMISSIVE) roles={public}
//     WITH CHECK: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = cadence_templates.organization_id)))
...
//     USING: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = deals.organization_id)))
//   Policy "Users can insert deals in their organization" (INSERT, PERMISSIVE) roles={public}
//     WITH CHECK: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = deals.organization_id)))
...
//     USING: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = knowledge_base_audios.organization_id)))
//   Policy "Users can insert kb audios in their organization" (INSERT, PERMISSIVE) roles={public}
//     WITH CHECK: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = knowledge_base_audios.organization_id)))
...
//     USING: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = knowledge_base_files.organization_id)))
//   Policy "Users can insert kb files in their organization" (INSERT, PERMISSIVE) roles={public}
//     WITH CHECK: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = knowledge_base_files.organization_id)))
...
//     USING: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = leads.organization_id)))
//   Policy "Users can insert leads in their organization" (INSERT, PERMISSIVE) roles={public}
//     WITH CHECK: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = leads.organization_id)))
...
//     USING: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = messages.organization_id)))
//   Policy "Users can insert messages in their organization" (INSERT, PERMISSIVE) roles={public}
//     WITH CHECK: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = messages.organization_id)))
...
//     USING: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = notifications.organization_id)))
//   Policy "Users can insert notifications in their organization" (INSERT, PERMISSIVE) roles={public}
//     WITH CHECK: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = notifications.organization_id)))
...
//     USING: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = products.organization_id)))
//   Policy "Users can insert products in their organization" (INSERT, PERMISSIVE) roles={public}
//     WITH CHECK: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = products.organization_id)))
...
//     USING: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = status.organization_id)))
//   Policy "Users can insert statuses in their organization" (INSERT, PERMISSIVE) roles={public}
//     WITH CHECK: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = status.organization_id)))
...
//     USING: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = whatsapp_instances.organization_id)))
//   Policy "Users can insert instances in their organization" (INSERT, PERMISSIVE) roles={public}
//     WITH CHECK: (auth.uid() IN ( SELECT users.id    FROM users   WHERE (users.organization_id = whatsapp_instances.organization_id)))
...
// Table: cadence_templates
//   CREATE UNIQUE INDEX cadence_templates_name_organization_id_key ON public.cadence_templates USING btree (name, organization_id)
...
//   CREATE UNIQUE INDEX products_name_organization_id_key ON public.products USING btree (name, organization_id)
// Table: staff_availability
...
//   CREATE UNIQUE INDEX status_name_organization_id_key ON public.status USING btree (name, organization_id)
// Table: users
...
//   CREATE UNIQUE INDEX whatsapp_instances_organization_id_key ON public.whatsapp_instances USING btree (organization_id)
// Table: whatsapp_webhooks
