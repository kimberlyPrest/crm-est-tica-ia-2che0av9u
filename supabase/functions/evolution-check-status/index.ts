...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!instance) {
      return new Response(
        JSON.stringify({ message: 'No active instances to check' }),
        {
          status: 200,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        },
    ...
    if (instance.last_checked_at) {
      ...
      const now = Date.now()
      if (now - lastChecked < 30000 && !instanceId) {
        // Only skip if auto-triggered
        console.log(
          `[Check] Skipping instance ${instance.instance_name}, checked recently.`,
        )
    ...
    try {
      ...
      if (response.status === 404) {
        ...
        await supabaseAdmin
          ...
          .eq('id', instance.id)
        return new Response(
          JSON.stringify({ status: 'failed', reason: '404' }),
          {
            status: 200,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          },
      ...
      if (response.status === 401) {
        ...
        await supabaseAdmin
          ...
          .eq('id', instance.id)
        return new Response(
          JSON.stringify({ status: 'failed', reason: '401' }),
          {
            status: 200,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          },
    ...
    if (state === 'open') {
      ...
      if (instance.connection_status !== 'connected') {
        ...
        try {
          ...
          if (instancesRes.ok) {
            ...
            const myInstance = instancesData.find(
              (i: any) => i.instance?.instanceName === instance.instance_name,
            )

            if (myInstance && myInstance.instance) {
              const { owner, profileName, profilePictureUrl } =
                myInstance.instance
              updates.phone_number = owner
                ? owner.split('@')[0]
      ...
      if (!instance.qr_code) {
        try {
          const connectRes = await callApi(
            `/instance/connect/${instance.instance_name}`,
            'GET',
          )
          if (connectRes.ok) {
