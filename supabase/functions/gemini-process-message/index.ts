        ...
        functionDeclarations: [
          {
            ...
            description:
              "Atualiza o estágio do CRM do lead atual baseado na intenção da conversa. Ex: 'Ser Humano', 'Qualificado', 'Agendado', 'Cliente'.",
            parameters: {
              type: 'OBJECT',
              properties: {
                status_nome: {
    ...
    if (candidate?.parts?.[0]?.functionCall) {
      ...
      if (functionCall.name === 'atualizar_status_lead') {
        ...
        if (statusObj) {
          ...
          functionResponse = {
            error: `Status ${statusName} não encontrado no CRM da organização.`,
          }
        }
