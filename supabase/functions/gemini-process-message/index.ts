...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [{ data: audios }, { data: files }] = await Promise.all([
      supabase.from('knowledge_base_audios').select('name, trigger_keywords').eq('is_active', true).eq('organization_id', orgId),
      supabase.from('knowledge_base_files').select('name').eq('is_active', true).eq('organization_id', orgId)
    ])
    ...
    enhancedContext += `\n\n--- REGRAS DE CRM E STATUS ---
Você é responsável por mover o lead pelo funil de vendas (Kanban). Baseado na intenção do cliente, atualize o status do lead usando a ferramenta "atualizar_status_lead".
Fluxo esperado:
- Dúvidas/Interesse inicial -> "Qualificado"
- Intenção ou solicitação de agendamento -> "Agendado"
- Confirmação de compra/presença -> "Cliente"
Avalie a mensagem recebida e chame a função "atualizar_status_lead" caso a intenção justifique o avanço.`
        ...
        functionDeclarations: [
          {
            ...
            description: "Atualiza o estágio do CRM do lead atual baseado na intenção da conversa. Ex: 'Ser Humano', 'Qualificado', 'Agendado', 'Cliente'.",
            parameters: {
              type: "OBJECT",
              properties: {
    ...
    if (candidate?.parts?.[0]?.functionCall) {
      ...
      if (functionCall.name === 'atualizar_status_lead') {
        ...
        const { data: statusObj } = await supabase.from('status').select('id').eq('organization_id', orgId).ilike('name', `%${statusName}%`).maybeSingle()
        if (statusObj) {
          ...
          functionResponse = { error: `Status ${statusName} não encontrado no CRM da organização.` }
        }
      }
      else if (functionCall.name === 'enviar_audio') {
        const audioName = functionCall.args.nome_audio
        const { data: audioRecord } = await supabase.from('knowledge_base_audios').select('audio_path').eq('organization_id', orgId).ilike('name', `%${audioName}%`).maybeSingle()
        ...
        const { data: fileRecord } = await supabase.from('knowledge_base_files').select('file_path').eq('organization_id', orgId).ilike('name', `%${fileName}%`).maybeSingle()
