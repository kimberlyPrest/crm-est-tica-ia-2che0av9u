...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Explicitly pass the token to getUser to bypass local session storage in Edge Functions
    const {
      data: { user },
      error: authError,
    ...
    await supabaseAdmin
      ...
      .update({
        ...
        qr_code: null,
        connection_error_message: apiSuccess
          ? 'Desconectado manualmente'
          : `Desconectado manualmente (Erro API: ${apiErrorMsg})`,
    ...
    if (!apiSuccess) {
      return new Response(
        JSON.stringify({ partialSuccess: true, error: apiErrorMsg }),
        {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        },
